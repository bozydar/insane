...
impl Error {
    fn new(message: &str, position: Position) -> Self {
        Self {
...
type BuildInFun = fn(Vec<Rc<Expr>>, Position) -> ExprResult;
...
impl Const {
    fn string(value: &str, position: Position) -> Const {
        Const {
    ...
    fn numeric(value: f64, position: Position) -> Const {
        Const {
    ...
    fn bool(value: bool, position: Position) -> Const {
        Const {
...
fn parse_sane(input: &str) -> ExprResult {
...
fn execute_sane(input: &str) -> ExprResult {
...
fn execute(expr: Rc<Expr>, stack: &mut Stack) -> ExprResult {
    ...
    match &*expr {
        ...
        Expr::Ident(ident) => {
            if let Some((_, expr)) = stack.iter().rev().find(|item| { &item.0 == &ident.label }) {
                Ok(expr.clone())
            } else {
                // Err(format!("Ident `{}` not found: {}", ident, stack_to_string(stack)))
                Error::new(&format!("Ident `{}` not found", ident.label), ident.position).into()
            }
        }
        Expr::Bind(Bind { args, fun, position }) => {
            fn build_curry_or_execute(current_args: Vec<Rc<Expr>>, current_fun: Rc<Expr>, position: Position, stack: &mut Stack) -> ExprResult {
                ...
                let params_as_idents = &mut params.iter()
                    .map(|param| {
                        Rc::new(Expr::Ident(Ident { label: param.clone(), position: position }))
                    })
                    .collect::<Vec<Rc<Expr>>>();
            ...
            build_curry_or_execute(arg_results, fun, position.clone(), stack)
        }
        ...
        Expr::List(List { items, position }) => {
            ...
            Ok(Rc::new(Expr::List(List { items: result, position: *position })))
        }
        Expr::Fun(fun) => {
